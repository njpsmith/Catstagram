...
class Searchbar extends React.Component {
  ...
  componentDidMount() {
    this.setState({
      categoryDataStringified: JSON.parse(JSON.stringify(catCategoryData)),
    });
  }
  ...
  getBreedIDByName = (breedName) => {
    const selectedBreed = this.state.categoryDataStringified.filter(
      (item) => item.name === breedName
    );
    ...
    return (
      <div className="searchbar">
        ...
        <Combobox
          openOnFocus
          height={40}
          items={categoryDataStringified.map((item) => item.name)}
